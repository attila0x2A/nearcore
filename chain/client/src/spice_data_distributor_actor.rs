...
macro_rules! debug {
    ($($arg:tt)*) => { tracing::debug!(target: "spice_data_distribution", $($arg)*) };
}
...
pub struct SpiceDataDistributorActor {
    ...
    // FIXME: Include parts.
    // FIXME:
    // note a TODO to not request past
    // execution head + 1 or something like that. (Which we'd know by talking to core state.)
    // This can be either when taking from or when adding to waiting_on_data.
    waiting_on_data: HashSet<SpiceDataIdentifier>,
    recent_produced_data: LruCache<SpiceDataIdentifier, SpiceData>,
}
...
impl SpiceDataDistributorActor {
    pub fn new(
        ...
        Self {
            ...
            // FIXME:
            recent_produced_data: LruCache::new(NonZeroUsize::new(2000).unwrap()),
        }
    }
    ...
    fn distribute_data(
        ...
        // FIXME: store parts?
        // FIXME: Avoid clone
        self.recent_produced_data.push(data_id.clone(), data.clone());
        ...
        let Some(SplitDataForDistribution {
            recipients,
            mut boxed_parts,
            mut merkle_proofs,
            commitment,
            part_ord,
        }) = self.split_data_for_distribution(me, &data_id, data)?
        else {
            return Ok(());
        };
            ...
            NetworkRequests::SpicePartialData {
                partial_data: SpicePartialData::new(
                    ...
                    vec![SpiceDataPart {
                        ...
                        part: boxed_parts[part_ord].take().unwrap(),
                        merkle_proof: merkle_proofs.swap_remove(part_ord),
                    }],
                    &signer,
                ),
                recipients,
    ...
    fn split_data_for_distribution(
        &mut self,
        me: &AccountId,
        data_id: &SpiceDataIdentifier,
        data: &SpiceData,
    ) -> Result<Option<SplitDataForDistribution>, Error> {
        ...
        Ok(Some(SplitDataForDistribution {
            recipients,
            boxed_parts,
            merkle_proofs,
            commitment,
    ...
    fn start_waiting_on_data(&mut self, block_hash: &CryptoHash) -> Result<(), Error> {
        ...
        for from_shard_id in shard_layout.shard_ids() {
            data_ids.push(SpiceDataIdentifier::Witness {
                block_hash: *block_hash,
                shard_id: from_shard_id,
            });
            for to_shard_id in shard_layout.shard_ids() {
                data_ids.push(SpiceDataIdentifier::ReceiptProof {
                    block_hash: *block_hash,
                    from_shard_id,
                    to_shard_id,
                });
        ...
        for data_id in data_ids {
            ...
            // FIXME: Include parts we are waiting on.
            if recipients.contains(me) {
                self.waiting_on_data.insert(data_id);
            }
        }
        Ok(())
    }
    ...
    fn request_waiting_on_data(&self) {
        ...
        for id in &self.waiting_on_data {
            // FIXME: Not sure about unwraps.
            let block = self.chain_store.get_block(id.block_hash()).unwrap();
            let (recipients, mut producers) = self.recipients_and_producers(&id, &block).unwrap();
            assert!(!producers.contains(me));
            assert!(!producers.is_empty());
            assert!(recipients.contains(me));
            // FIXME: Request only the missing parts, each from exactly one producer.
            // (Or have a TODO to do some of it later.)
            self.network_adapter.send(PeerManagerMessageRequest::NetworkRequests(
                NetworkRequests::SpacePartialDataRequest {
                    // FIXME: Sign?
                    data_id: id.clone(),
                    // FIXME: Pick 1 at random
    ...
    fn handle_partial_data_request(
        ...
        // FIXME: create a helper and new error type for this
        let Some(signer) = self.validator_signer.get() else {
            return Err(Error::Other("no validator signer to request waiting on data"));
        };
        let me = signer.validator_id();
        let Some(SplitDataForDistribution {
            recipients: _,
        ...
        {
            // FIXME: Avoid 1 extra clone; use repeat_n?
...
//FIXME: Move up?
struct SplitDataForDistribution {
    recipients: HashSet<AccountId>,
    boxed_parts: Vec<Option<Box<[u8]>>>,
    merkle_proofs: Vec<Vec<near_primitives::merkle::MerklePathItem>>,
